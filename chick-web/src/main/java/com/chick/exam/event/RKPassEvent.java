...
public class RKPassEvent implements ExamReptileEvent {
    ...
    public static void main(String[] args) {
        String url = "http://www.rkpass.cn//tk_timu/4_739_1_xuanze.html";
        String cookie = "JSESSIONID=71D681859A35BBF7F6A21DF0CC82EEBC_系统架构师_综合知识_第一题";
        Document document = null;
        try {
            document = Jsoup.parse(HttpUtil.createGet(url)
                    .header("Cookie", cookie.split("_")[0]).execute().toString());
        ...
        examQuestionDetailVO.setExamFiles(examFiles);
        examQuestionDetailVO.setExamKnowledges(examKnowledges);
        System.out.println(examQuestionDetailVO);
    }
}
